...
# ====================================================
# TAB 1: My Tasks (simple placeholder)
# ====================================================
with tabs[0]:
    st.subheader("📋 My Tasks")
    st.info("This section shows your active and completed tasks. (To be connected with manager assignments.)")

# ====================================================
# TAB 2: AI Feedback Summarization (Auto from Reviews)
# ====================================================
with tabs[1]:
    ...
    if st.button("🔍 Load & Analyze Feedback"):
        if not company or not employee:
            st.warning("⚠️ Please enter both company name and your name.")
        else:
            ...
            if df.empty:
                ...
                if polarity > 0.2:
                    ...
                    suggestion = "Keep up the great work and maintain consistency."
                elif polarity < -0.2:
                    sentiment = "⚠️ Overall Negative Feedback"
                    suggestion = "Focus on addressing the issues highlighted and seek clarification."
                else:
                    sentiment = "💬 Neutral Feedback"
                    suggestion = "Mixed reviews. Continue to communicate and improve where needed."

                st.markdown("---")
                st.markdown(f"### 📊 Sentiment Analysis Result")
                st.markdown(f"**Sentiment:** {sentiment}")
                st.markdown(f"**Polarity Score:** `{polarity:.2f}` | **Subjectivity:** `{subjectivity:.2f}`")
